...
class RemoteExperienceMaker(ABC):
    def __init__(
        ...
        custom_rollout_filter:str=None,
        ...
        **kwargs,
        ...
        self.train_llm = strategy.args.train_llm
        ...
        self.custom_rollout_filter = None
        if custom_rollout_filter:
            if not custom_rollout_filter.endswith(".py"):
                raise ValueError(f"custom_rollout_filter must be a python file, got {custom_rollout_filter}")
            print(f"Loading custom `rollout_filter(self,rollout_samples)` from {custom_rollout_filter}")
            import importlib.util

            spec = importlib.util.spec_from_file_location("rollout_filter", custom_rollout_filter)
            rollout_filter_module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(rollout_filter_module)
            self.custom_rollout_filter = rollout_filter_module.rollout_filter

    @torch.no_grad()
    def make_experience_list(
        self, all_prompts: Union[str, List[str]], all_labels, rollout_buffer=None, **generate_kwargs
    ) -> List[Experience]:
        """
        ...
        if self.custom_rollout_filter:
            rollout_samples = rollout_samples.split(1, self.data_processor)
            rollout_samples = self.custom_rollout_filter(self, rollout_samples)
            rollout_buffer.add_samples(rollout_samples)
            if not rollout_buffer.full():
                logger.info(f"📌 Rollout buffer space: {len(rollout_buffer)}/{rollout_buffer.limit}. Continue to sample more data.")
                return None
            else:
                logger.info(f"📌 Rollout buffer space: {len(rollout_buffer)}/{rollout_buffer.limit}. Stored buffers will be sampled for experience making.")
                rollout_samples = rollout_buffer.get_buffered_samples()
        ...
        experiences, count_filter_0, count_filter_1 = self.compute_advantages_and_returns(experiences)
        return experiences, count_filter_0, count_filter_1
        # return experiences
    ...
    def make_experience(self, rollout_samples: Samples) -> List[Experience]:
        ...
        logger.info(f"✨ Starting rewards calculation")
        ...
        logger.info(f"✨ Starting action logprobs calculation")
        ...
        if self.critic_model_group is not None:
            logger.info(f"✨ Starting value calculation")
        ...
        if self.initial_model_group is not None:
            logger.info(f"✨ Starting ref action logprobs calculation")
            ...
            base_action_log_probs_ref = ray.put(
                [[None]] * (len(samples_list) * args.ring_attn_size * args.ds_tensor_parallel_size_ref)
            )
        ...
        duplicate_factor_ref = args.ring_attn_size * args.ds_tensor_parallel_size_ref
        action_log_probs_list = sum(ray.get(action_log_probs_ref)[::duplicate_factor], [])
        base_action_log_probs_list = sum(ray.get(base_action_log_probs_ref)[::duplicate_factor_ref], [])
    ...
    def compute_advantages_and_returns(
        ...
        if self.custom_experience_filter:
            experiences, count_filter_0, count_filter_1 = self.custom_experience_filter(self,experiences)
        ...
        # 如果没有启用 custom_experience_filter 统计全0全1的reward，那么这一步就来统计
        if not self.custom_experience_filter:
            count_filter_0 = sum(torch.all(t == 0) for t in rewards)
            count_filter_1 = sum(torch.all(t == 1) for t in rewards)
    ...
    def generate_samples(self, all_prompts: List[str], all_labels, **generate_kwargs) -> Samples:
        ...
        """
        start_time = time.time()
        logger.info(f"🚀 Starting sample generation with {len(all_prompts)*generate_kwargs.get('n_samples_per_prompt', 1)} prompts")

        if self.vllm_engines is None:
            samples = self._generate_with_hf(all_prompts, all_labels, **generate_kwargs)
        else:
            # vLLM generation
        ...
        duration = end_time - start_time
        time_str = str(timedelta(seconds=duration)).split(".")[0]
        logger.info(f"✨ Sample generation completed in {time_str}")
        return samples
        ...
        for i, llm in enumerate(llms):
            ...
            if messages:
                ...
                if self.train_llm:
                    ...
                    images = [self.data_processor.get_images_from_messages_cached(m) for m in messages]
                    vllm_inputs = [{
                            "prompt": p,
