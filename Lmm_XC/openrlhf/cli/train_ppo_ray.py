...
def train(args):
    ...
    if args.ds_tensor_parallel_size != args.ds_tensor_parallel_size_ref:
        _args = copy.deepcopy(args)
        _args.ds_tensor_parallel_size = args.ds_tensor_parallel_size_ref
        ref_strategy = get_strategy(_args)
    else:
    ...
    if args.vllm_num_engines is not None and args.vllm_num_engines > 0:
        ...
        if args.colocate_all_models and args.colocate_vllm:
            assert (
        ...
        vllm_engines = create_vllm_engines(
            ...
            args.enforce_eager,
            max_len,
            pg if args.colocate_all_models and args.colocate_vllm else None,
    ...
    if args.init_kl_coef <= 0:
        ...
        ref_model = PPORayActorGroup(
            ...
            pg=pg,
            num_gpus_per_actor=0.2 if pg else 1,
            duplicate_actors=args.ring_attn_size * args.ds_tensor_parallel_size_ref,
        )
    ...
    if ref_model is not None:
        refs.extend(ref_model.async_init_model_from_pretrained(ref_strategy, args.pretrain))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no_colocate_vllm", dest="colocate_vllm", action="store_false", default=True)
    ...
    parser.add_argument("--ds_tensor_parallel_size_ref", type=int, default=None, help="DeepSpeed tensor parallel size for reference model")
    ...
    parser.add_argument("--filter_overlong_prompt", action="store_true", default=False, help="Filter overlong prompt")
    ...
    parser.add_argument("--custom_rollout_filter", type=str, default=None, help="Custom rollout filter")
    # Custom dataset
    ...
    parser.add_argument(
        "--wandb_run_name",
        type=str,
        default="run_%s" % datetime.now().strftime("%m%dT%H%M"),
    )
    ...
    # XC parameters
    parser.add_argument("--train_llm", action="store_true", default=False, help="Train LLM, default is VLM")
    ...
    if args.ds_tensor_parallel_size_ref is None:
        args.ds_tensor_parallel_size_ref = args.ds_tensor_parallel_size
