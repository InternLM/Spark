...
def reward_func(queries, prompts, labels):
    ...
    for query, prompt, label in zip(queries, prompts, labels):
        ...
        if len(gold_parsed) != 0:
            ...
            try:
                reward = float(is_equal(math_answer, gold_parsed, 'None', math_mode="math_verify"))
                if reward == 0.0:
                    reward = float(is_equal(math_answer, gold_parsed, 'None', math_mode="legacy"))
                    if reward == 0.0:
                        if math_answer == gold_parsed:
                            reward = 1.0
            ...
            if minute % 10 == 0:
                with open(log_path, "a") as f:
                    ...
                    f.write(f"math_answer: {math_answer}\n")
